...
# 1) download acid-------
## 2013 - 2014 ----
file_datasus13_14 <- "../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOEXT_2013_to_2014.rds"
...
file_INFdatasus13_14 <- "../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOINF_2013_to_2014.rds"
...
## 2015 - 2019 ----
file_datasus15_19 <-"../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOEXT_2015_to_2019.rds"
...
file_INFdatasus15_19 <-"../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOINF_2015_to_2019.rds"
...
dt_merge <- rbind(filter_datasus("../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOINF_2013_to_2014.rds"),
                  filter_datasus("../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOEXT_2013_to_2014.rds"),
                  filter_datasus("../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOINF_2015_to_2019.rds"),
                  filter_datasus("../transporte_ativo_2008-2019/data-raw/datasus/SIM-DOEXT_2015_to_2019.rds"))
...
dt_merge <- dt_merge[,list(total_acid = sum(num_acid,na.rm = TRUE))
                     ,by = .(code_muni_sus
                             ,causa_name,year)]
dt_merge[,sum(total_acid),by = .(year)]
...
ibge_2010 <- readr::read_rds("../transporte_ativo_2008-2019/data-raw/sidrar/pop_2010_sexo_age_raca.rds")
...
dt_pop <- readr::read_rds("../transporte_ativo_2008-2019/data-raw/sidrar/pop_2013_to_2019.rds")

# 5) Merge data -----

## . w/ metro name & pop data ----
tmp_dt_acid <-  data.table::merge.data.table(
  x = dt_merge  ,y = dt_pop  ,by = c('code_muni_sus','year')
)
...
dados_acid <- rbind(
  ...
  tmp_dt_acid[,list("deaths" = sum(total_acid,na.rm = TRUE))
              ,by = .(municipio_codigo,causa_name,year)],
  # only active
  tmp_dt_acid[causa_name %in% c('walk','bike'),list(
    "deaths" = sum(total_acid,na.rm = TRUE))
    ,by = .(municipio_codigo,year)][,causa_name := "walk+bike"],
  # total
  tmp_dt_acid[,list("deaths" = sum(total_acid,na.rm = TRUE))
              ,by = .(municipio_codigo,year)][,causa_name := "total"]
)
...
tmp_pop_rm <- copy(tmp_dt_acid)[,.SD[1],by = .(municipio_codigo, municipio_codigo,year)]
tmp_pop_rm <- tmp_pop_rm[,list(pop = sum(valor)),by = .(municipio_codigo,year)]

dados_acid <- dados_acid[tmp_pop_rm,on = c("municipio_codigo","year")]
...
## . w/ IBGE 2010 prop----
ibge_2010[,prop := pop / sum(pop,na.rm = TRUE)
          ,by = .(municipio_codigo,year)]

ibge_2010[,sum(prop),by = .(municipio_codigo,year)]
ibge_2010$AGE %>% table(useNA = "always")
ibge_2010$sexo %>% table(useNA = "always")
ibge_2010$AGE %>% table(useNA = "always")
ibge_2010[1]
dados_acid[1]
dados_acid[ibge_2010,on = c("municipio_codigo"),prop := i.prop]

# checking
dados_acid[is.na(prop)]

# save data ----
readr::write_rds(dados_acid,"data/datasus/muni_by_mode.rds")
readr::write_rds(dados_acid,"data/datasus/muni_by_mode_age_cor_sexo.rds")
