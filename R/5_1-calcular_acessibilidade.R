...
calcular_acess_muni <- function(sigla_muni, ano, BFCA = FALSE, mode_access = "all", indicator_access = "all") {
  ...
  if (mode_access == "todos") {
    ...
    path <- sprintf("../r5r/routing/%s/muni_%s/ttmatrix_all_%s_%s_r5.csv",
                          ano, sigla_muni, ano, sigla_muni)
  ...
  dir_hex <- sprintf("../data/dados_hex/muni_%s/dados_hex_%s.rds",
                     sigla_muni, sigla_muni)
  ...
  if (indicator_access == "all") {
    ...
    #ainda precisa dos dados socioesconômicos 
