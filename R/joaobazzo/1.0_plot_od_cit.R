...
zt <- zt %>% 
  melt.data.table(id.vars = c("zt","geometry")
                  ,measure.vars = c('o_tp', 'd_tp', 'o_ti', 'd_ti'))
...
o_ti_plot <- plot_f(dt = zt_sf
                    ...
                    ,myfill = "Número de\n viagens"
                    ,mycap = NULL)
d_ti_plot <- plot_f(dt = zt_sf
                    ,var = "d_ti"
                    ,pal = "E"
                    ,subtitle = "Destino"
                    ,myfill = "Número de\n viagens"
                    ,mycap = NULL)
o_tp_plot <- plot_f(dt = zt_sf
                    ,var = "o_tp"
                    ,pal = "A"
                    ,subtitle = "Origem"
                    ...
                    ,myfill = "Número de\n viagens"
                    ,mycap = NULL)
d_tp_plot <- plot_f(dt = zt_sf
                    ,var = "d_tp"
                    ,pal = "B"
                    ,subtitle = "Destino"
                    ,myfill = "Número de\n viagens"
                    ,mycap = "Fonte: Adaptado do Plano de Mobilidade Urbana (LOGIT, 2013).")

pf <- ((o_ti_plot + d_ti_plot) / (o_tp_plot + d_tp_plot) ) + 
  plot_annotation(tag_levels = 'I',tag_prefix = "(",tag_suffix = ")") + 
  plot_layout(nrow = 2, byrow = TRUE)

ggplot2::ggsave(pf,filename = "figures/bra_cit/ti_tp_od.png",
                scale = 0.85,width = 30,height = 25,
                bg = "white",
                units = "cm",dpi = 300)
# save
