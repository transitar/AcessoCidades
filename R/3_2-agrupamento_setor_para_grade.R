...
renda_de_setor_p_grade <- function(ano, munis = "all") {
  ...
  renda_de_setor_p_grade_muni <- function(sigla_muni) {
    ...
    path_setor <- sprintf("../data-raw/setores_censitarios/%s/setores_%s_%s.rds", ano, sigla_muni, ano)
    ...
    #adição das informações dos setores
    
    path_infos <- sprintf("../data-raw/censo_2021_info_muni_treated/muni_%s.rds", sigla_muni)
    info_setor <- read_rds(path_infos) %>% mutate(Cod_setor = as.character(Cod_setor))
    
    setor <- left_join(setor, info_setor, by = c("code_tract" = "Cod_setor")) %>% 
      rowwise() %>%
      mutate(PTOT = sum(across(starts_with("P0")))
    )
    
    setor <- subset(setor, PTOT > 0)
    # mapview(grade, zcol = "POP")
    # mapview(setor, zcol = "PTOT")
    ...
    grade_corrigida2 <- grade_corrigida %>%
      group_by(id_grade) %>%
    ...
    mapview(grade_corrigida)
    
    # Criar id unico de cada setor e filtra colunas DO SETOR
    # calcula area de cada setor
    setor2 <- setor %>%
      mutate(id_setor = 1:n()) %>% st_as_sf()
    
    
    setor2$area_setor <- st_area(setor2)
    
    setor2 <- setor2  %>%
      dplyr::select(id_setor, area_setor,PTOT, 
    ...
    setDT(setor2)[,  ":="(
      ...
      # Homen por cor
      cor_b_h_prop = P001/PTOT,
      cor_p_h_prop = P002/PTOT,
      cor_a_h_prop = P003/PTOT,
      cor_pa_h_prop = P004/PTOT,
      cor_i_h_prop = P005/PTOT,
      
      # Mulheres por cor
      
      cor_b_m_prop = P006/PTOT,
      cor_p_m_prop = P007/PTOT,
      cor_a_m_prop = P008/PTOT,
      cor_pa_m_prop = P009/PTOT,
      cor_i_m_prop = P010/PTOT
    ), 
    ...
    ui <- sf::st_intersection(grade_corrigida, setor2)
