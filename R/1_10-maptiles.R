...
baixar_map_tile_ceramic <- function(ano, sigla_muni) {
  ...
  if (aprox_muni == 1){
    temp_sf <- read_sf(sprintf('../data-raw/dados_municipais_recebidos/muni_%s/muni_%s.gpkg',
                                    tolower(sigla_muni)
                                    ,tolower(sigla_muni)), layer = "areas") %>%
      st_make_valid() %>% st_union() %>% nngeo::st_remove_holes()
  ...
  # temp_sf <- read_municipality(code_muni = muni1,year = 2019) 
    ...
    if (aprox_muni == 1){
      ...
      readr::write_rds(tab, sprintf("../data/maptiles_crop/%s/mapbox_2/maptile_crop_mapbox_%s_%s_aprox.rds", ano, sigla_muni, ano))
